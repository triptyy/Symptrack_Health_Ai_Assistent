...
const apiKey = 'AIzaSyD-ZKUKNZLIVeNFZppSTJ9D8_x7Nq_G2JY';

app.post('/find-doctors', async (req, res) => {
    let { latitude, longitude, disease } = req.body;
    console.log(`📡 Received request for doctors near (${latitude}, ${longitude}) for disease: ${disease}`);
    disease = disease.split(",")[0].toLowerCase();
    console.log(`CHOSEN DISEASE : --> ${disease}`);
